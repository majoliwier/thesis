...
{
  /* LTDC, PLLSAI i framebuffer są już skonfigurowane w main.c
   * (Twój kod z LCD_FRAMEBUFFER, HAL_LTDC_Init, HAL_LTDC_ConfigLayer itd.)
   * Tutaj tylko podpinamy driver i ustawiamy właściwości rysowania.
   */

  /* Wybierz urządzenie (sterownik ILI9341) */
  LcdDrv = &ili9341_drv;

  /* SDRAM – bezpiecznie można zainicjalizować drugi raz */
  BSP_SDRAM_Init();

  /* Domyślna warstwa: 0 */
  ActiveLayer = 0;

  /* Domyślne właściwości rysowania */
  DrawProp[ActiveLayer].BackColor = LCD_COLOR_BLACK;
  DrawProp[ActiveLayer].TextColor = LCD_COLOR_WHITE;
  DrawProp[ActiveLayer].pFont     = &LCD_DEFAULT_FONT;
...
{
  uint32_t xaddress = 0;
  uint32_t bytesPerPixel = (LtdcHandler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565) ? 2 : 4;

  /* Get the line address */
  xaddress = (LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress) + bytesPerPixel * (BSP_LCD_GetXSize() * Ypos + Xpos);
...
void BSP_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
  if(LtdcHandler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565)
  {
    // Konwersja ARGB8888 -> RGB565
    uint16_t rgb565 = ((RGB_Code & 0xF80000) >> 8) |  // R: 5 bitów
                      ((RGB_Code & 0x00FC00) >> 5) |  // G: 6 bitów
                      ((RGB_Code & 0x0000F8) >> 3);   // B: 5 bitów

    /* Write data value to SDRAM memory (RGB565 = 2 bytes per pixel) */
    *(__IO uint16_t *)(LtdcHandler.LayerCfg[ActiveLayer].FBStartAdress +
                       (2 * (Ypos * BSP_LCD_GetXSize() + Xpos))) = rgb565;
  }
  else
  {
    /* Write data value to SDRAM memory (ARGB8888 = 4 bytes per pixel) */
...
{
  uint32_t colorMode;
  uint32_t finalColor = ColorIndex;

  /* Determine color mode and convert color if needed */
  if(LtdcHandler.LayerCfg[LayerIndex].PixelFormat == LTDC_PIXEL_FORMAT_RGB565)
  {
    colorMode = DMA2D_OUTPUT_RGB565;
    /* Convert ARGB8888 -> RGB565 */
    finalColor = ((ColorIndex & 0xF80000) >> 8) |  // R
                 ((ColorIndex & 0x00FC00) >> 5) |  // G
                 ((ColorIndex & 0x0000F8) >> 3);   // B
  }
  else
  {
  ...
  /* Register to memory mode with correct color mode */
  Dma2dHandler.Init.Mode         = DMA2D_R2M;
  Dma2dHandler.Init.ColorMode    = colorMode;
  ...
  if (HAL_DMA2D_Init(&Dma2dHandler) == HAL_OK)
  {
    ...
    {
      if (HAL_DMA2D_Start(&Dma2dHandler, finalColor, (uint32_t)pDst, xSize, ySize) == HAL_OK)
      {
